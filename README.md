...
End-to-end pipelines for **Regression**, **Classification**, and **Clustering** (B.Tech CSE, Academic Year 2026–27). This README focuses on the **Regression track**, which is the Review 1 deliverable in `notebooks/regression.ipynb`.
...
The same cleaned, encoded, scaled table is sent to **all ten required algorithms**. They are scored on the **same held-out test trips**. Results are compared in one table (R², RMSE, MAE). The two best official models get 5-fold cross-validated R². A manual tester lets you type in a trip. A final cluster writes **`results/regression_report.pdf`** with the plots and metrics.
...
├── requirements.txt
├── data/
│   └── nyc_taxi_trip_duration.csv    # 352,000 rows, 45 MiB (under GitHub's 50 MiB warning)
├── notebooks/
...
└── app/                              # optional GUI (Review 2 bonus)
```
...
Python 3.9+ on macOS (Apple Silicon works). From the repo root:
...
In Cursor / VS Code open `notebooks/regression.ipynb`, choose the **`.venv`** kernel (top-right), then **Run All**.
...
| 2 | Ridge | L2; handles collinear distances |
| 3 | Lasso | L1; watch how many weights become 0 |
| 4 | ElasticNet | L1 + L2 |
| 5 | Polynomial | `PolynomialFeatures` degree 1 vs 2, then linear |
| 6 | Decision Tree | Depth cap + feature importance |
| 7 | Random Forest | Ensemble of trees |
| 8 | Gradient Boosting | sklearn `HistGradientBoostingRegressor` (same family, runnable at this scale) |
| 9 | SVR | RBF + scaled target; trained on 12k rows |
| 10 | KNN | k = 15; trained on 40k rows |
...
Do not upload the whole notebook in one commit.

1. Dataset + audit / EDA  
2. Cleaning + feature engineering  
3. Linear family (Linear, Ridge, Lasso, ElasticNet, Polynomial)  
4. Tree family (DT, RF, GBM)  
5. SVR + KNN  
6. Comparison, tuning, diagnostics, CV  
7. Manual tester + PDF export  

---

## Classification and clustering

Those tracks are Review 2 (`notebooks/classification.ipynb`, `notebooks/clustering.ipynb`). They are not in this folder yet. Keep the same repo rules: one notebook per track, `random_state=42`, train-only scalers.

---

## Academic note

Course guidelines allow generative AI for scaffolding, not for invented interpretation. Plot comments in the notebook are tied to statistics computed from `data/nyc_taxi_trip_duration.csv`. Cite AI assistance in this README if the team used it for code structure.
